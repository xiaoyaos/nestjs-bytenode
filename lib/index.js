 ...
 * @param * 
 */
const compileMain = function(rootDir = null, originDir = './dist', {nodeModulesPack = false}){
  let absoluteOriginDir = path.join(rootDir, originDir);
  if(!rootDir){
    throw new Error(`rootDir must be a string`);
  }
  let generateDir = path.join(rootDir, generateDirName);
...
function compileDir(dir) {
  ...
  if (stat.isFile() ) {
    if(path.extname(dir) == '.js'){
      ...
      console.log("*-*-*-*-*-*-*-*", output)
      const exist = fs.existsSync(path.dirname(output))
      if(!exist){
      ...
      // fs.unlinkSync(dir);
    }else if(path.extname(dir) == '.json'){
      const output = dir.replace('dist', generateDirName);
      const exist = fs.existsSync(path.dirname(output))
      if(!exist){
        mkdirp.sync(path.dirname(output));
      }
      fs.copyFileSync(dir, output)
    }
